class Solution {
public:
    bool canPlace(vector<vector<char>>& board, int row, int col, int n, char ch) {
        ...
        for(int row = 0 ; row < board.size() ; row++)
    {
        for(int col = 0 ; col < board[0].size() ; col++)
        {
            if(board[row][col] == '.')
            {
                ...
                {
                    bool isSafe = canPlace(board , row , col , 9, ch) ;
                    ...
                    if(isSafe)
                    {
                        board[row][col] = ch ;
							...
							if(solveSudokuUtil(board))  return true ;       //recursive call
